...
const tag = '[face/register]';

export async function POST(req: NextRequest) {
  try {
    const { accountId, selfieBase64 } = await req.json();
    ...
    console.log(`${tag} selfiePrefix=${selfieBase64?.slice(0, 50)}`);
    ...
    console.log(`${tag} url=${baseUrl}/api/v1/face/register`);
    console.log(`${tag} bodyBytes=${bodyJson.length}`);
    console.log(`${tag} bodyShape=${JSON.stringify({ accountId, ...FACE_DEVICE, selfieList: [{ imageType: 'SN', fileBytes: selfieBase64?.length }] })}`);

    const res = await fetch(`${baseUrl}/api/v1/face/register`, {
      method: 'POST',
      headers,
      body: bodyJson,
    });
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          error: `Face register failed: ${res.status}`,
          detail: data,
          _debug: {
            url: `${baseUrl}/api/v1/face/register`,
            accountId,
            selfieBytes: selfieBase64?.length ?? 0,
            selfieBase64Full: selfieBase64 ?? null,
            bodyShape: { accountId, ...FACE_DEVICE, selfieList: [{ imageType: 'SN' }] },
          },
        },
